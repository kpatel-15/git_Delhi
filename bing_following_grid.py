...
bingFile = '/nobackup/users/patel/Segments//bing_shape_name.txt'
...
#ax = Axes(fig, [.1,.1,.8,.8])        
#for i in range(10000):#(len(k)):
##for i in range(10):
#    (line1_xs, line1_ys) = zip(*k[i])
#    
#    fig.add_axes(ax)     
...
for i in range(len(k)):
    knew.append(pyproj.transform(wgs84,myProj,list(zip(*k[i]))[1], list(zip(*k[i]))[0]))
