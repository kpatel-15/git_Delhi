...
for name in (glob.glob(sourcepath + '/special//traffic_1514937661*')): #1511861720 1514982661
    print('\n Reading file: \t', name)
    try:
        ...
        if out_fun.minute == 0:
            print ('\n Accepted file with timestamp',  ntpath.basename(name), out_fun)
            top_DE = []
            top_LI = []
            le = []
            de = []
...
# Average by time
hpulse = df.groupby(['time', 'item_description', 'item_length']).mean()
